...
function get_description()
	return {"READ VALUES FROM IN.N AND IN.B",
			"COMPUTE LOG(N) BASE B ROUNDED DOWN",
			"WRITE TO OUT.LOG" }
end
...
function get_streams()
	...
	output = {}
	for i = 1,39 do
		--inputn[i] = math.floor(math.pow(10, 3*math.random()))
		inputn[i] = math.floor(math.pow(10, 0.5+2.5*math.random()))
		inputb[i] = math.random(2, 10)
		
		
		--doing log like this is impresise because of floating point math
		--output[i] = math.floor( math.log(inputn[i])/math.log(inputb[i]) )
