...
function get_description() -- I don't know how to better explain a Linear Feedback Shift Register in the space available
	return {
		"READ A 9 BIT SEED NUMBER FROM IN",
...
function get_streams()
	...
	if seed == 233 then
		seed = 429
	end -- 429 first run long runs of 0s and 1s and alternating 0s and 1s should trigger most edge cases
	input[1] = seed
	x = input[1]

	for i = 1, 39 do
		if
			(math.floor(x / 256) ~= 0 or math.floor((x % 32) / 16) ~= 0)
			and not (math.floor(x / 256) ~= 0 and math.floor((x % 32) / 16) ~= 0)
		then
			x = (x % 256) * 2 + 1
			output[i] = 1
		else
